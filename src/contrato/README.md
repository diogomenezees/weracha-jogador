...
| `tipos.ts` | `weracha-site/docs/especificacao/16-api-v1.md` (`Parceiro`) + `weracha-site/app/api/v1/**` + `weracha-site/lib/auth-tipos.ts` + `weracha-site/lib/jogadoresSeguro.ts` (`MeuPerfil`) + `weracha-site/lib/services/exclusaoConta.ts` (`DadosDaTelaExclusaoConta`) + `weracha-site/lib/artilheiros.ts` (`DadosArtilheiros` e tipos) + `weracha-site/lib/services/resenha.ts` (`FeedResenha`, `BlocoFeedResenha`, `ComentarioResenha`) + `weracha-site/lib/services/enquetes.ts` (`Enquete`, `EnquetesDoGrupo`, …) + `weracha-site/lib/services/{membros,perfilJogador}.ts` + `lib/db/schema.ts` (`MembroGrupo`, `PosicaoEsporte`, `CheckIn`, `ConfiguracaoPartida`, `PreAlocacaoPartida`, `CorGrupo`, `ResultadoSalvo`, `EstadoAoVivo`) + `weracha-site/lib/services/checkins.ts` (`DadosDeApoioDaPartida`, `TipoPagamento`) + `weracha-site/lib/services/aoVivo.ts` (`EstadoAoVivoCompleto`, `GolComVideos`, `MeuReplay`) | Request/response das rotas que o app consome (inclui `StatusTelefone`/`EnvioCodigoSms` do fluxo de acesso e o ciclo da partida) |
